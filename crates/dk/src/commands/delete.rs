...
pub fn delete_single(data_dir: &Path, id: Uuid) -> std::io::Result<()> {
...
#[allow(clippy::missing_errors_doc)]
pub fn run_delete(data_dir: &Path, args: &[String]) -> std::io::Result<()> {
    if args.is_empty() {
        let state = CliState::load(data_dir);
        let id = state
            .current
            .ok_or_else(|| std::io::Error::other("no current item set"))?;
        return delete_single(data_dir, id);
    }
